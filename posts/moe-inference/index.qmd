---
title: "Mixture-of-Experts Inference: parallelism, communication, and load balancing"
date: "2026-07-24"
...
![Expert dispatch and combine without overlap and with two-batch overlap. Splitting the batch into two micro-batches lets one compute while the other communicates, so the compute stream stays busy. Red blocks are compute, blue blocks are dispatch and combine.](images/two-batch-overlap.png){width=90%}
...
![Expert load before and after EPLB replication. Each GPU has two expert slots, one occupied and one redundant. The MoE layer cannot finish until the busiest GPU is done, so the popular expert E0 on GPU 0 leaves the others idle. Placing copies of E0 into the redundant slots spreads its tokens across four GPUs and finishes the layer earlier.](images/eplb-slots.png){width=90%}
