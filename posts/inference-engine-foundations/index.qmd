...
- **Weight quantization** -- reducing model weights from FP16 to INT8, INT4, or even lower bit-widths cuts memory footprint proportionally. A 70B-parameter model at FP16 needs ~140 GB; at INT4, it fits in ~35 GB. This is the difference between needing 4 GPUs and needing 1.
...
**Data parallelism (DP)** gives each GPU a full model replica, with different GPUs handling different requests. It is simple and effective when the model fits on one GPU and you just want to scale throughput.
