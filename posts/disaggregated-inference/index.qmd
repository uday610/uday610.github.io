...
The Kubernetes scheduler should ensure **physical proximity** during the initial pod placement. Prefill and decode pods need to sit physically close to each other (same rack, zone, and so on) so that KV cache transfers between them stay fast and low-latency. This is handled by topology-aware gang scheduling.
